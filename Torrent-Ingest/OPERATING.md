...
This replaces `~/Developer/diagnosis.txt`, which was a hand-off note written for whatever
...
```bash
bash ~/Developer/Torrent-Ingest/scripts/verify_fleet.sh      # 40 blocking checks
```
...
`~/Developer/HANDOFF.md` §1 and §3. The short version: use
...
wc -l < ~/Developer/Media-Syncer/mediafs_deletions.jsonl
...
python3 ~/Developer/Torrent-Ingest/scripts/reconcile_library_db.py --apply --include-requested
```
...
`grep -ic '<Title>' ~/Developer/Media-Syncer/remote_inventory.json` count down to 0 rather
...
for q in ('mediafs_deletions.jsonl', 'mediafs_deletions.jsonl.processing'):
    f = os.path.expanduser('~/Developer/Media-Syncer/' + q)
