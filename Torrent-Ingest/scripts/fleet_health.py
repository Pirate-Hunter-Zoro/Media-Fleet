...
MEDIA_SYNCER_DIR = Path.home() / "Developer" / "Media-Orchestrator" / "Media-Syncer"
...
def _expected_labels() -> set:
    ...
    found = set()
    try:
        for p in (Path.home() / "Developer" / "Media-Orchestrator").glob("*/com.mikeyferguson.*.plist"):
            found.add(p.stem)
    except OSError:
        return set()
...
def check_vpn() -> list[tuple[str, str]]:
    ...
    """
    try:
        sys.path.insert(0, str(Path.home() / "Developer" / "Media-Orchestrator" / "Media-Syncer"))
...
def check_acceptance_gate() -> list[tuple[str, str]]:
    ...
    """
    try:
        sys.path.insert(0, str(Path.home() / "Developer" / "Media-Orchestrator" / "Torrent-Ingest"))
