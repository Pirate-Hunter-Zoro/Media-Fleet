...
#
# Run at the end of a session. Prefer scripts/ship.sh, which also restarts the
# daemons that run this repository's code -- a push alone leaves every running
# daemon on the old code, which is invisible from the outside.
#
