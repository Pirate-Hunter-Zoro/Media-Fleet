#!/bin/bash
# Launcher invoked by launchd. Pulls any config edits, then runs the manga
# chapter/volume reconciler once. `--scheduled` drains queued map refreshes,
# refreshes stale maps for two-tier series, and applies the purges the cached
# maps prove -- this is the intermittent scan the chapter policy asks for.
set -euo pipefail
...
exec "$PYTHON" "$SCRIPT_DIR/scripts/chapter_volume_reconcile.py" --scheduled
