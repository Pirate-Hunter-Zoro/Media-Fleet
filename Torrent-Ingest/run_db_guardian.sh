#!/bin/bash
# Launcher invoked by launchd. Pulls any config edits, then execs the guardian
# under its conda env. The daemon runs its own loop, so launchd only needs to
# (re)start this if it ever dies.
set -euo pipefail
...
exec "$PYTHON" "$SCRIPT_DIR/db_guardian.py"
