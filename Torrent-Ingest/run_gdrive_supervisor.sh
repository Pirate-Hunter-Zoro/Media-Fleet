#!/bin/bash
# Launcher invoked by launchd. Pulls any config edits, then execs the Google Drive
# supervisor under its conda env. Runs its own loop; KeepAlive restarts it only if it dies.
set -euo pipefail
...
exec "$PYTHON" "$SCRIPT_DIR/gdrive_supervisor.py"
