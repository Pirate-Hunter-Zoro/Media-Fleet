...
| `scripts/test_supervisor_yacreader.py` | The supervisor's YacReader contract: start with flags patched and marker consumed, bounce on drift, activate a windowless app (alerting after five tries), back off after three rapid crashes, and hide after every fleet start/activate through a bounded window. Blocking check. |
