...
| **Media-Syncer** (`~/Developer/Media-Orchestrator/Media-Syncer`) | replication to the MEGA pool, the `mediafs` virtual library, pre-download and eviction | the sync cycle, the purge/rename runbooks, the mount, the split tunnel |
| **YouTube-Downloader** (`~/Developer/Media-Orchestrator/YouTube-Downloader`) | YouTube discovery and download, placed through **this repo's** plan API | discovery, download waves, routing, the soundtrack split |
...
### YouTube ingest (`~/Developer/Media-Orchestrator/YouTube-Downloader`, daemon `com.mikeyferguson.youtubesync`)
...
plists** (`~/Developer/Media-Orchestrator/*/com.mikeyferguson.*.plist`), not hand-listed — a hand-maintained
