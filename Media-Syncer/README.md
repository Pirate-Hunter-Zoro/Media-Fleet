...
> **Single-host by assumption, not by configuration.** There is no host-mode toggle and no staging role: every path assumes the Mini and its SSD library root. Other machines may `git pull` this repo to receive `rclone.conf` changes, but **only the Mini runs the daemon.**
...
| **Torrent-Ingest** (`~/Developer/Media-Fleet/Torrent-Ingest`) | acquisition and placement: torrents, GetComics, drive ingest, the plan API, Jellyfin health | the ingest state machine, `library.validate_plan`/`apply_plan`, the reaper, playlists |
| **YouTube-Downloader** (`~/Developer/Media-Fleet/YouTube-Downloader`) | YouTube discovery and download, placed through Torrent-Ingest's plan API | discovery, download waves, routing, the soundtrack split |


Media-Syncer is the **replication layer** of a small multi-daemon fleet on the Mini. Its upstream is **Torrent-Ingest** (`~/Developer/Media-Fleet/Torrent-Ingest`), the sibling daemon that turns "drop a `.torrent` into an iCloud folder" into "the title is correctly named, correctly placed, and watchable" — video organized to Jellyfin/TMDB conventions, comics/manga to YACReader conventions — landing every file on the SSD library root (~/Media). Everything Torrent-Ingest writes to the SSD library root this daemon then sees, on its next cycle, as a new local file absent from every remote and **replicates out to the MEGA pool**. Local Jellyfin playback never waits on that upload; the MEGA copy is durability, not a serving path. **YouTube-Downloader** (`~/Developer/Media-Fleet/YouTube-Downloader`) is the fleet's third daemon: it discovers and downloads YouTube content and places it through Torrent-Ingest's plan API, so a YouTube video reaches the pool through the same door a torrent does.
...
A single `rclone.conf` is checked into the repo and propagated across the fleet via git. All remotes with `type = mega` are treated as part of the storage pool — there is no namespacing or filtering.

**Never commit `session_id` or `master_key` lines.** Only the durable credentials (`user`, `pass`, `use_trash`) belong in the committed `rclone.conf`. `session_id`/`master_key` are ephemeral, machine-local MEGA auth tokens that rclone regenerates on demand from `user`/`pass` — committing them is pointless and actively harmful: a cached token is tied to the machine that minted it, so propagating it across the fleet hands every peer a session that is stale-on-arrival, surfacing as the `go-mega` SIGSEGV panic / `couldn't login` failures the daemon then has to self-heal from (see *Stale-session recovery*). It also silently defeats a hand-run maintenance delete: rclone can exit `0` while the operation actually failed on the dead session, so a delete looks like it succeeded when the file is still there. The daemon's own `purge_mega_session()` only ever strips these tokens from the local `~/.config/rclone/rclone.conf`, never the repo copy, precisely to keep them out of git. If they ever creep back into the committed file, strip every `session_id`/`master_key` line before committing.
...
2. **Config refresh** — `git pull` at the top of every loop iteration, picking up any `rclone.conf` changes (e.g. new MEGA remotes) pushed from another machine in the fleet. The freshly-pulled `rclone.conf` is then copied into `~/.config/rclone/rclone.conf` so rclone actually sees the new remotes — without this deploy step, the pull is cosmetic
...
If the config refresh yields zero usable remotes (missing or empty `rclone.conf`), the loop skips the cycle and `continue`s — the next iteration's `git pull` gets another chance to repair the situation. The daemon is therefore **self-healing** against transient config loss.
...
* **The config-pull remote must be SSH, not HTTPS.** The daemon runs headless under launchd, so git has no TTY to prompt on: with an HTTPS origin and no credential helper, every top-of-cycle `git pull` dies with `fatal: could not read Username for 'https://github.com': Device not configured` and config propagation silently stalls. Use an SSH remote — `git@github.com:Pirate-Hunter-Zoro/Media-Syncer.git` — backed by `~/.ssh/id_ed25519`, which authenticates non-interactively. If a clone ever arrives on HTTPS, fix it with `git remote set-url origin git@github.com:Pirate-Hunter-Zoro/Media-Syncer.git`.

* **Git-driven config propagation:** `rclone.conf` is versioned in the repo. Adding a MEGA account on any host is a matter of editing the conf, committing, and pushing — peer machines pick it up on their next cycle via the top-of-loop `git pull`, which also redeploys the conf into `~/.config/rclone/` so rclone immediately sees the change. **That pull is serialized behind an `flock` on `.git/pull.lock`, and naming the refspec is not a substitute for it.** Two pulls against one working tree each append a mergeable line to the shared `.git/FETCH_HEAD`, so the second to read it sees two candidates for `main` and dies with `fatal: Cannot fast-forward to multiple branches` however precisely it named `origin main` — measured at 5 of 6 concurrent raw pulls failing against a scratch clone, and 0 of 6 through the lock. There is only one pull site here, so the overlap takes two *processes*, which is exactly what a restart produces: launchd SIGTERMs the outgoing daemon and starts the incoming one while it is still finishing its iteration. On contention the pull is **skipped, not queued** — the process holding the lock is pulling the same tree, so the update lands regardless, and a cycle must never block on config propagation. A missed pull is not free here: a cycle runs for days, so "it retries next cycle" can mean days of running on a stale `rclone.conf`

* **The tree lock covers the account provisioner's commit, not just pulls.** Serializing pull-against-pull leaves pull-against-*local-commit* wide open, and that is the gap that actually bites, because `mega_accounts.create_accounts()` writes the working tree from a different thread whenever the pool runs low. The provisioner appends a remote to `rclone.conf`, then `git add`/`commit`/`push`es it — and a pull landing anywhere inside that sequence aborts. Two distinct shapes, depending on exactly where it lands: between the append and the commit it dies with `error: Your local changes to the following files would be overwritten by merge`, and during the commit itself with `fatal: Cannot fast-forward your working tree`, whose accompanying advice to run `git reset --hard` would **discard the in-flight provisioner commit** if anyone followed it. Measured while the pool provisioned 31 accounts in one burst: **4 aborted pulls in 12 minutes.**

  So both sites share one `flock` on `.git/pull.lock` via `utils.git_tree_lock(block=...)`, and the two callers take it with deliberately different miss-policies:

  * The **pull** passes `block=False` and skips, exactly as before — the holder is either pulling this same tree or committing to it, so the tree ends up current either way.
  * The **provisioner** passes `block=True` and waits, because its commit is not optional: skipping it would leave a registered MEGA account present in the working tree but never pushed, so no other host ever learns the account exists.

  Registration sits deliberately **outside** the lock. It takes minutes (MEGA signup plus an IMAP confirmation round-trip), and holding a git lock across it would block every pull on the machine for the length of a whole batch. Only the append-through-push window is serialized. The wait is **bounded** (180 s, comfortably above `config.GIT_PULL_TIMEOUT_SEC`'s 120 s) so a legitimately running pull always releases in time, while a wedged holder cannot stall provisioning forever — past the timeout the provisioner proceeds unlocked and logs it, which is the old behaviour and never loses an account.
...
* **Git-as-control-plane:** `rclone.conf` updates propagate across machines via `git push` / `git pull`; each cycle begins with a pull so new remotes become usable on the very next iteration
...
A multi-threaded macFUSE filesystem presenting a **merged** view of `lower` (`Media` — real files: metadata sidecars + hot media, **read-write passthrough** so Jellyfin's `.nfo`/artwork writes land on the real drive) and the inventory (every media payload, presented **full-size** so `stat`/scans are instant and never touch a remote). Scoped to `config.MEDIAFS_PREFIXES` (`Shows/`/`Movies/`/`Comics/`). Bytes are fetched **only on an actual read**. Runs as `com.mikeyferguson.mediafs`; `run_mediafs.sh` waits for `Media` (the SSD library root populated) before mounting, force-cleans a stale mount, and `KeepAlive` remounts on any death.
...
When the pool's **placeable** free space — each account capped at the 20 GB free tier (never trust promo space above that) and reduced by the per-claim `REMOTE_FILL_MARGIN_BYTES`, with excluded remotes dropped — falls below `POOL_LOW_FREE_BYTES` (200 GB), the daemon creates new free accounts automatically: `megatools reg` → read the confirmation email over IMAP → verify → prove login → append the account to `rclone.conf` (password rclone-obscured, **never** session tokens) → commit + push so the whole fleet picks it up. New accounts are named `automega<n>` with the email alias `<base>+automega<n>@…` (Gmail/iCloud `+`-aliases all land in the one base inbox). Measuring *placeable* rather than *aggregate* free is what stops the pool stalling while it still looks empty — see *Pool capacity provisioning* above. If the next `automega<n>` alias turns out to already be registered at MEGA (an orphan from a prior run that died before it could commit — `_next_index()` only sees `rclone.conf`, not MEGA's registry, so it can't know), registration returns `EEXIST` and the daemon **skips to the next alias** instead of aborting the batch, bounded by `ACCOUNTS_MAX_PER_RUN` so a pathological run of orphans can't spin forever.

**It is inert (monitor-only) until an IMAP app-password is present** at `~/.config/media-syncer/email_app_password` (a Gmail **App Password** — 16 lowercase letters — *not* the account password, which can't do IMAP under 2FA; the file lives **outside the repo** and must never be committed — it bypasses 2FA and is far more sensitive than the throwaway MEGA creds in `rclone.conf`). Without it, the system just logs when capacity is low.
...
The same script pins a **second** network around the exit node, for the YouTube ingest (`~/Developer/Media-Fleet/YouTube-Downloader`). Mullvad exits are commercial-VPN **datacenter** IPs, which YouTube rate-limits and `403`s far more aggressively than a residential address — each failure wasting a download wave.
...
> `~/Developer/Media-Fleet/YouTube-Downloader` **refuses to run a cycle at all** while Google egresses
