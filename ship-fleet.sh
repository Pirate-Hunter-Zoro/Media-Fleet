...
#
#      bash ~/Developer/Media-Orchestrator/ship-fleet.sh "commit message"
#      bash ~/Developer/Media-Orchestrator/ship-fleet.sh --restart-only
#
...
_self="${BASH_SOURCE[0]}"
while [ -L "$_self" ]; do _self="$(readlink "$_self")"; case "$_self" in /*) ;; *) _self="$HOME/Developer/Media-Orchestrator/$_self";; esac; done
...
# searches ~/Developer/Media-Orchestrator/*/ for a matching plist, so there is nothing left for it to revive.
#
